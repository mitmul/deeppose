...
def get_roi(img, joint):
    ...
    return img, (st_x, st_y)


def get_target_joints(joint, lt):
    joint = joint.tolist()
    joint_pos = []
...
def save_crop_images_and_joints():
    ...
    for i, joint in enumerate(joints):
        ...
        img, lt = get_roi(img, joint)
        joint_pos = get_target_joints(joint, lt)

        if np.all(joint_pos > 0):
            img = cv.resize(img, (227, 227))
            cv.imwrite('data/lspet_dataset/crop/im%05d.jpg' % (i + 1), img)
            joint_pos /= np.array([img.shape[1], img.shape[0]])
